<template>
  <form class="relative flex justify-between">
    <div class="w-[45%]">
      ...
      <button type="button" class="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:focus:ring-yellow-900" @click="updateForm">Update</button>
    </div>
    <div class="w-[45%]">
      <div class="mb-4">
        <label
...
const addedGenres = ref<Genre[]>([])
const selected = ref<Genre[]>(props.genres)
const genre = ref<''>()
const coverUrl = ref<string>(props.topViewUrl)
const imageUrl = ref<string>(props.topViewUrl || '/image/default.jpg')
...
const updateForm = async () => {
  ...
  const { error } = await useFetch<string>(`/api/manga/${route.params.id}`, {
    method: 'put',
...
const handleSelect = (newSelected: Genre[]) => {
  selected.value = newSelected.filter((item, index, self) => self.findIndex(t => t.id === item.id) === index)
}
